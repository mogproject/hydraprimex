...
TEST(VertexSeparatorTest, FindVertexSeparator) {
  ...
  auto g2 = TriGraph(4, {{0, 1}, {0, 2}, {1, 2}, {1, 3}, {2, 3}});
  ...
  auto g3 = TriGraph(4, {{0, 1}, {0, 2}, {1, 2}, {1, 3}, {2, 3}, {0, 3}});  // K_4
  ...
  auto g4 = TriGraph(8, {{1, 2}, {1, 4}, {5, 2}, {5, 4}, {7, 2}, {7, 4}, {6, 2}, {6, 4}, {0, 2}, {0, 4}, {3, 2}, {3, 4}});
...
TEST(VertexSeparatorTest, DecomposeIntoCovers) {
  ...
  EXPECT_EQ(ret5_1[0].number_of_vertices(), 8);
  auto ret5_2 = sep.decompose_into_covers(g5, 2);
  EXPECT_EQ(ret5_2.size(), 2);
  for (int i = 0; i < 2; ++i) EXPECT_EQ(ret5_2[i].number_of_vertices(), 5);
  auto ret5_3 = sep.decompose_into_covers(g5, 3);
  EXPECT_EQ(ret5_3.size(), 8);
  for (int i = 0; i < 2; ++i) EXPECT_EQ(ret5_3[i].number_of_vertices(), 3);
  ...
  EXPECT_EQ(ret6_1[0].number_of_vertices(), 12);
  auto ret6_2 = sep.decompose_into_covers(g6, 2);
  EXPECT_EQ(ret6_2.size(), 1);
  EXPECT_EQ(ret6_2[0].number_of_vertices(), 12);
  auto ret6_3 = sep.decompose_into_covers(g6, 3);
  EXPECT_EQ(ret6_3.size(), 3);
  for (int i = 0; i < 3; ++i) EXPECT_EQ(ret6_3[i].number_of_vertices(), 6);

  //----------------------------------------------------------------------------
  auto g7 = gen::path_graph(10);
...
TEST(VertexSeparatorTest, FindArticulationPoints) {
  ...
  TriGraph c5(5, {{0, 1}, {1, 2}, {2, 3}, {3, 4}, {4, 0}});
  EXPECT_EQ(sep.find_articulation_points(c5), VI());

  TriGraph g1(5, {{0, 1}, {0, 2}, {0, 3}, {0, 4}});
  EXPECT_EQ(sep.find_articulation_points(g1), VI({0}));

  TriGraph g2(5, {{1, 0}, {1, 2}, {1, 3}, {1, 4}});
  EXPECT_EQ(sep.find_articulation_points(g2), VI({1}));

  TriGraph g3(7, {{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}, {0, 4}, {0, 5}, {0, 6}, {4, 5}, {4, 6}, {5, 6}});
  ...
  TriGraph g7(10, {{0, 1}, {1, 2}, {3, 4}, {4, 5}, {6, 7}, {6, 8}});
