...
class SATAdapter {
 ...
 public:
  SATAdapter(util::Random *rand = nullptr) : status_(0), top_id_(0), num_clauses_(0) {
  ...
  int solve(int time_limit_sec = 0) {
    // log_trace("SATAdapter solving: num_vars=%d, num_clauses=%d, time_limit=%ds", top_id_, num_clauses_, time_limit_sec);
    if (time_limit_sec > 0) {
